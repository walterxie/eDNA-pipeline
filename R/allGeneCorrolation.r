...
getMantelAndProcrustes <- function(input.names, metric="jaccard",
  ...
  dissim <- ComMA::getDissimilarityList(cm.prep.list, metric=metric)
...
plotMantelAndProcrustes <- function(corrs, text.repel=T, label.digits=3, 
                                    gene.levels=c("16S", "18S", "26S", "ITS", "COI-300", "COI-650")) {
  require(ggplot2)
  theme_set(theme_bw(base_size=8))
  
  m.p.tri <- corrs$corrs
  p.hm <- ComMA::plotCombinedTriMatrix(m.p.tri, label.digits=label.digits, data.levels=gene.levels,
                                       #lower.lim=0.58, upper.lim=1.0, breaks.by=0.1,
                                       title="", legend.title="Correlations")
  
  p.m.mds <- ComMA::nmdsTriMatrix(dist(corrs$mantel.tri), text.or.point=1, text.size=5, 
                                  text.repel=text.repel, title="Mantel test") + 
    expand_limits(x = c(-1, 1), y=c(-1, 1))
  
  p.p.mds <- ComMA::nmdsTriMatrix(dist(corrs$prot.tri), text.or.point=1, text.size=5, 
                                  text.repel=text.repel, title="Procrustes test") + 
    expand_limits(x = c(-1, 1), y=c(-1, 1))
