...
envdata_bysubplot <- read.table("H:/My Documents/PhD Research PFR folder/LBI_miseq_analysis_stuff/LBI_U8_OTUs_analyses/LBI_environmental_data/LBI_all_env_data_by_subplot.txt", sep="\t", header=T, row.names=1)
...
#g <- c("18S")
genelist <- c("16S", "18S", "26S", "ITS", "FolCO1", "ShCO1")
#metric <- "Jaccard"
#metric <- "1-beta1"

for (g in genelist){
  
  #f <- Sys.glob(paste0("OTUtables_norm/", g, "*otutable_normalized.txt"))
  f <- Sys.glob(paste0("OTUtables_norm/", g, "*otutable_min2_normalized.txt"))
  ...
  tableFile <- paste0("LBI_RDA/LBI_", g, "_RDA_tables_2.txt")
  logfile <- paste0("LBI_RDA/LBI_", g, "_RDA_log.txt_2")
  
  ### Get taxonomic data ###
  match <- Sys.glob(paste0("Taxa_tables/", genelabel, "*s_nt_paths_new_taxonomy_table.txt"))
  taxaTable <- read.table(match, sep="\t", header=T, row.names=1, quote = "", comment.char = "")
  taxa <- taxaTable[,c("Superkingdom","Kingdom","Phylum","Class")]
  #taxa$OTU <- rownames(taxa)
  OTUtable <- merge(df, taxa, by = "row.names")
  rownames(OTUtable) <- OTUtable$Row.names
  OTUtable$Row.names <- NULL
  
  # Split into taxonomic subsets
  OTUsubset <- subset(OTUtable, !(grepl("root|cellular organisms|No hits|Not assigned", OTUtable$Kingdom)))
  Proks <- subset(OTUsubset, grepl("PROKARYOTA", OTUsubset$Superkingdom))
  Euks <- subset(OTUsubset, grepl("EUKARYOTA", OTUsubset$Superkingdom))
  Fungi <- subset(OTUsubset, grepl("FUNGI", OTUsubset$Kingdom))
  Metazoa <- subset(OTUsubset, grepl("ANIMALIA", OTUsubset$Kingdom))
  Plants <- subset(OTUsubset, grepl("PLANTAE", OTUsubset$Kingdom))
  Protists <- subset(OTUsubset, grepl("CHROMISTA|PROTOZOA", OTUsubset$Kingdom))
  
  taxa.subsets <- list("eukaryotes" = Euks,  
                       "protists" = Protists, "fungi" = Fungi, 
                       "animals" = Metazoa, "plants" = Plants, "prokaryotes" = Proks)
  ...
  for(i in 1:length(taxa.subsets)){
    ...
    print(paste(g, taxon))
    OTUtable1 <- as.data.frame(taxa.subsets[[i]])
    print(paste("OTU rows:", nrow(OTUtable1)))
    OTUtable1$Superkingdom <- NULL
    OTUtable1$Kingdom <- NULL
    OTUtable1$Phylum <- NULL
    OTUtable1$Class <- NULL
    
    cat(paste("\n\n", g, taxon, "\n"), file = logfile, append = T)
    cat(paste("\n", g, taxon, "Number of rows and columns before filtering:", nrow(OTUtable1), ncol(OTUtable1), "\n"), file = logfile, append = T)
    print(dim(OTUtable1))
    
    if(nrow(OTUtable1) > 0){  # Not an empty table
      print(colSums(OTUtable1))
      cat(paste("\n", g, taxon, "colSums before filtering:\n"), file = logfile, append = T)
      write.table(as.data.frame(colSums(OTUtable1)), file = logfile, sep = "\t", quote = F, append = T)
      
      x <- mean(colSums(OTUtable1))
      OTUtable1 <- as.data.frame(OTUtable1[, colSums(OTUtable1 != 0) > 5])  # Exclude any samples with fewer than x OTUs
      OTUtable1 <- as.data.frame(OTUtable1[, colSums(OTUtable1) > x*0.025])  # Exclude any samples with excessively low abundance
      OTUtable1 <- as.data.frame(OTUtable1[rowSums(OTUtable1) > 0, ]) # Exclude any empty rows    
      
      cat(paste("Number of rows and columns after filtering:", nrow(OTUtable1), ncol(OTUtable1), "\n"), file = logfile, append = T)
      print(dim(OTUtable1))
      cat(paste("\n", g, taxon, "colSums after filtering:\n"), file = logfile, append = T)
      write.table(as.data.frame(colSums(OTUtable1)), file = logfile, sep = "\t", quote = F, append = T)
      
      if((nrow(OTUtable1) > 25) & (ncol(OTUtable1) > 28)) {{ # Ignore tables with few remaining samples/OTUs 
        cat(paste("More than x rows: proceed with capscale", "\n\n"), file = logfile, append = T)
        
        OTUtable.t <- t(OTUtable1) # Transpose
        if((nrow(OTUtable.t) != nrow(envdata))) { # Rows don't match
          both <- match(rownames(OTUtable.t), rownames(envdata)) # Matching rows
          envdata1 <- envdata[both, ] # Subset to matching rows
        }else if((nrow(OTUtable.t) == nrow(envdata))) { # Rows match
          envdata1 <- envdata # Continue
        }
        
        communityMatrix <- OTUtable.t
        ...
        # Code borrowed from Walter's pipeline
        ...
        # DB-RDA, empty model
        rda_0 <- capscale(communityMatrix ~ 1, envdata1, distance = "jaccard")
        
        # DB-RDA, maximal model (bad idea - only use for auto model building)
        rda_1 <- capscale(communityMatrix ~ ., envdata1, distance = "jaccard")
        ...
        print(xtable(anova_table, caption = paste("Distance-based redundancy analysis and their ANOVA tests 
                                            in each step for the eDNA biodiversity data sets", g, taxon), 
                     label = paste("tab:rdaAnova", g, taxon, sep = "_"), caption.placement = "top"), 
        ...
        print(xtable(rda_table, caption = paste("The constrained and unconstrained inertia changes 
                                          during distance-based redundancy analysis for the eDNA biodiversity data sets", g, taxon), 
                     label = paste("tab:rda", g, taxon, sep = "_"), caption.placement = "top"), 
              sanitize.text.function = function(x){x}, file=tableFile, append=TRUE)
        
        rdaReducedList[[ taxon ]] <- rda_reduced
        rdaForwardList[[ taxon ]] <- rda_forward
        rdaBackwardList[[ taxon ]] <- rda_backward
      }
    }
  }  # End of for each taxon subset
    
  # Make biplots using ggplot ------------------------------------------------
  rdaLists <- list("Reduced" = rdaReducedList, "Forward" = rdaForwardList, "Backward" = rdaBackwardList)
  i <- 1
  for(rdaList in rdaLists){
    rdaPlots <- list()
    lab <- names(rdaLists)[[ i ]]
    j <- 1
    for(rda in rdaList){
      ...
      p <- ggplot(data = sites) + geom_point(aes(x = sites[,1], y = sites[,2], colour = Elevation), shape = 1, size = 2, alpha = 0.6) +
                  ...
                  geom_text(data = biplots, aes(x = biplots[,1]*2.2, y = biplots[,2]*2.2, label = rownames(biplots)), size = 4, colour = "blue", alpha = 0.75) +
                  xlab("") + ylab("") +
                  ...
                  #coord_fixed() +
                  ggtitle(paste0(letters[j], ". ", g, " ", taxon, ", Jaccard distance RDA"))
      legend <- get_legend(p)  # Get legend
      
      p <- p + theme(legend.position = "none")
      
      rdaPlots[[j]] <- ggplotGrob(p)  
      j <- j + 1
    } # End of for each rda in rdaList
    ### Print plots, with size and layout dependent on number of plots ###
    if(length(rdaPlots) == 1){
    
      pdf(file = paste0("LBI_RDA/LBI_", g, "_min2_jaccard_RDA_", lab, "_plots_2.pdf"),
          width = 11/2.54, height = 9/2.54, useDingbats = FALSE)
      print (grid.arrange(rdaPlots[[1]], legend, nrow = 1, ncol=2, widths=c(1, 0.2)))
      dev.off()
      
    }else if(length(rdaPlots) == 2){
      
      pdf(file = paste0("LBI_RDA/LBI_", g, "_min2_jaccard_RDA_", lab, "_plots_2.pdf"),
          width = 20/2.54, height = 9/2.54, useDingbats = FALSE)
      args.list <- c(rdaPlots, list(ncol=2, nrow=1))
      print (grid.arrange(do.call(arrangeGrob, args.list), legend, ncol=2, widths=c(1, 0.1)))
      dev.off()
      
    }else if(length(rdaPlots) > 2 && length(rdaPlots) < 5){
      
      pdf(file = paste0("LBI_RDA/LBI_", g, "_min2_jaccard_RDA_", lab, "_plots_2.pdf"),
          width = 20/2.54, height = 18/2.54, useDingbats = FALSE)
      args.list <- c(rdaPlots, list(ncol=2, nrow=2))
      print (grid.arrange(do.call(arrangeGrob, args.list), legend, ncol=2, widths=c(1, 0.1)))
      dev.off()
      
    }else if(length(rdaPlots) > 4 && length(rdaPlots) < 7){
      
      pdf(file = paste0("LBI_RDA/LBI_", g, "_min2_jaccard_RDA_", lab, "_plots_2.pdf"),
          width = 20/2.54, height = 27/2.54, useDingbats = FALSE)
      args.list <- c(rdaPlots, list(ncol=2, nrow=3))
      print (grid.arrange(do.call(arrangeGrob, args.list), legend, ncol=2, widths=c(1, 0.1)))
      dev.off()
      
    }else if(length(rdaPlots) > 6 && length(rdaPlots) < 9){
      
      pdf(file = paste0("LBI_RDA/LBI_", g, "_min2_jaccard_RDA_", lab, "_plots_2.pdf"),
          width = 20/2.54, height = 36/2.54, useDingbats = FALSE)
      args.list <- c(rdaPlots, list(ncol=2, nrow=4))
      print (grid.arrange(do.call(arrangeGrob, args.list), legend, ncol=2, widths=c(1, 0.1)))
      dev.off()
    }
  i <- i + 1
  } # End of for each rdaList 
  }
