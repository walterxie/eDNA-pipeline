...
OTUs <- list()
i <- 1
for(gene in c("16S","18S","26S","ShCO1")){
  df <- read.table(Sys.glob(paste0("OTUtables/", gene, "*otutable_min2_by_plot.txt")), sep = "\t", header = TRUE, row.names = 1, check.names = F) 
  OTUs[[i]] <- df
  i <- i + 1
}
#OTUs <- rbindlist(OTUs) # Drops rownames
OTUs <- do.call(rbind, unname(OTUs)) # Retains rownames
OTUs$CM30b51 <- NULL # Drop sample with no veg data

### Load vegetation data ###
veg.df <- read.table(Sys.glob("../LBI_veg_data/LBI_Trees_Saplings_SBA.txt"), sep = "\t", header = TRUE, row.names = 1, check.names = F)
...
### Combine OTUs and veg data ###
OTUs <- rbind(OTUs, veg.df)

### Function to calculate (mean) elevation of maximum abundance for each OTU ###
get.elev <- function(OTU){
  OTU.row <- OTUs[rownames(OTUs) == OTU, ]
  site <- colnames(OTU.row)[OTU.row == max(OTU.row)]
  #if(length(site) > 1){
  #  print("Multiple equal sites")
  print(site)
  matches <- paste(site, collapse="|")
  elev <- mean(envdata.byplot$Elevation[grep(paste0("^", matches, "$"), rownames(envdata.byplot))])
  print(elev)
  return(elev)
}
...
setwd(paste0("H:/My Documents/PhD Research PFR folder/LBI_miseq_analysis_stuff/LBI_U8_OTUs_analyses/Network_analysis/Networks_by_OTUs_all_genes_veg_v3_phy_20-80_a"))#, l, "/"))
...
edges.mb$lty <- sapply(edges.mb$weight, function(x) ifelse(x < 0, 2, 1))
edges.mb$lty2 <- sapply(edges.mb$weight, function(x) ifelse(x < 0, "b", "a"))

### Get elevation values for each OTU ###
verts.mb$elev <- ""
verts.mb$elev <- mapply(get.elev, verts.mb$OTU)
summary(verts.mb$elev)
#verts.mb$elev.group <- 
...
#ppal <- iwanthue(length(unique(verts.mb$elev)))

p <- ggnet2(g, node.color = V(g)$Gene, palette = palette.gene, node.alpha = 0.5, #node.label = V(g)$elev, 
       node.size = V(g)$degree, 
       label.size = 2, label.alpha = 0.5,
       edge.size = abs(E(g)$weight*5), edge.alpha = 0.5, edge.lty = E(g)$lty,
       #edge.color = sign.mb,
...
### Plot network with nodes coloured by elevation using ggnetwork ###
g1 <- ggnetwork(g)

ggplot(g1, aes(x = x, y = y, xend = xend, yend = yend)) +
  #geom_edges(aes(linetype = lty2, size = weight, colour = lty2)) +
  geom_edges(aes(linetype = lty2, size = abs(weight*5)), alpha = 0.5, colour = "grey") +
  geom_nodes(aes(color = elev, size = degree), alpha = 0.5) +
...
ggplot(d[d$Gene != "Vegetation",], aes(x = comm, fill = Phylum)) + geom_bar() + facet_grid(Gene~., scales = "free") +
