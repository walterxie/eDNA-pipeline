...
#rankLevel="family" 
#groupLevel="order" # gives colour, and must higher than rankLevel
rankLevelList = c("class","class","order","order","order","family")
groupLevelList = c("phylum","phylum","class","class","class","order")
...
for (belongId in 1:length(belongToList)) {
	...
	for (expId in 1:length(matrixNames)) {
		...
		if (nrow(taxaPaths) < minReads) {
			print( paste(matrixNames[expId], " does NOT have >=", minReads, "reads classified as", belongTo, ", jump to next in the loop.") )	
			next
		}
...
#TODO: not working:	taxaAssg <- aggregate(as.formula(paste(".", rankLevel, sep=" ~ ")), data= taxaAssgReads, function(x) d(t(x),lev="gamma",q=1))	
	...
	#		}		
		...
		pdf(paste(workingPath, "figures/", filePrefix, "-", belongTo, "-", matrixNames[expId], "-", rankLevel, ".pdf", sep = ""), 
