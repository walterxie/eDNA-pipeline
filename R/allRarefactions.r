...
pdf(paste(workingPath, figDir, "/all-rarefaction-", otuThr, ".pdf", sep = ""), width=6, height=9)
attach(mtcars)
...
for (dFId in c(2,5,3,6,1,4)) { #length(levels)
    ...
    for (expId in 1:length(matrixNames)) {
        ...
        inputRDT <- paste(workingPath, matrixNames[expId], "/", matrixName, "-", otuThr, "-rarefaction-table.csv", sep = "")
