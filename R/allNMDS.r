...
getNMDS <- function(input.names, metric="jaccard", by.plot=FALSE,
  ...
  for (i in 1:length(cm.by.subplot.list)) {
    ...
    gg <- ComMA::ggNMDSPlot(t(cm.by.subplot.list[[i]]), env.subplot, colour.id="Elevation", shape.id="Forest.code", link.id="Plot", 
                            ...
                            title = paste0(letters[i], ". ", names(cm.by.subplot.list)[i], ", ", ComMA::simpleCap(metric), "distance"),
...
plotAllNMDS <- function(input.names, genes.taxa.list) {
  ...
  for (i in 1:length(genes.taxa.list)) {
    ...
    nmds <- getNMDS(input.names, genes.taxa=genes.taxa.list[[i]] )
    gt <- ComMA::grid_arrange_shared_legend(nmds$plot.list, input.list=T, nrow=3, 
