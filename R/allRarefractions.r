...
pdf(paste(workingPath, "figures/all-rarefraction-", otuThr, ".pdf", sep = ""), width=6, height=9)
attach(mtcars)
...
for (dFId in c(2,5,3,6,1,4)) { #length(levels)
    ...
    for (expId in 1:length(matrixNames)) {
        ...
        inputRDT <- paste(workingPath, experiments[expId], "/", matrixName, "-", otuThr, "-rarefraction-table.csv", sep = "")
	
		rarefractionTable <- read.csv(file=inputRDT, head=TRUE, sep=",", row.names=paste(levels, qs, sep=""), check.names=FALSE)
		 # remove prefix size.
	    sampleSizesSeq <- gsub("^.*?\\.","",colnames(rarefractionTable)) 
	...
	for (expId in 1:length(matrixNames)) {
		...
		if (expId == 1) {
			...
			#pdf(paste("figures/all-rarefraction-", otuThr, "-", paste(levels, qs, sep="")[dFId], ".pdf", sep = ""))
	        par(mar=c(4,4,3,2))
			plot(sampleSizesSeq, rarefractionTable[dFId,], pch=myshape[expId], col=mypalette[expId], xlim=c(0,maxX), ylim=c(0,maxY), 
				   xlab=xlab, ylab=paste(levels, "-", qs, " diversity", sep="")[dFId], main=subTitles[dFId]) 				  
			
		} else {
			points(sampleSizesSeq, rarefractionTable[dFId,], pch=myshape[expId], col=mypalette[expId]) 
		}
	
		lines(sampleSizesSeq, rarefractionTable[dFId,], lty=2, col=mypalette[expId]) 
	}    
