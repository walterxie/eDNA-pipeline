...
for(sS in sampleSizesSeq){   
   if (is.null(rdiversityTableMean)) {
	...
	print(paste("Rarefraction take sample size ", sS, ", max = ", sampleSizesSeq[length(sampleSizesSeq)], sep="")) 
}
...
print(paste("write rarefraction table file to ", outputRFTable, sep=""))
