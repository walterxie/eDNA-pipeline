...
for (expId in 1:length(experiments)) {	
	...
	source("RarefactionDiversitiesTable.R", local=TRUE)

	# create rarefaction table file which is calculated by RarefactionDiversitiesTable.R
	outputRFTable <- paste(workingPath, experiments[expId], "/", matrixName, "-", otuThre, "-rarefaction-table.csv", sep = "")
	source("Rarefaction.R", local=TRUE) 
} 
